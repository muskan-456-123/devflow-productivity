...
export function DreamDeskHUD(props: HUDProps) {
  ...
  const apiStateLabel = props.apiState === "connected" ? "Live workspace" : props.apiState === "loading" ? "Syncing workspace" : props.apiState === "empty" ? "No projects yet" : props.apiState === "error" ? "Offline · local view" : "Demo workspace";
  return <div className="dreamdesk-hud" data-api-state={props.apiState}>
    <header className="dream-topbar pointer-events-auto"><div className="dream-brand"><img src="/manus-storage/dreamdesk-mark_be24247f.png" alt="DreamDesk" /><div><strong>DreamDesk</strong><span>PRODUCTIVE ABOVE THE CLOUDS</span></div></div><div className="dream-topbar-right"><div className={`dream-api-status is-${props.apiState}`} role="status" aria-live="polite"><span className="dream-api-dot" /><strong>{apiStateLabel}</strong></div><div className="dream-profile"><span className="profile-orb">AC</span><div><strong>Ava Chen</strong><small>in a good rhythm</small></div></div></div></header>
